...
(comment
  ...
  (p/sym-at-or-before? {:bl 1 :bc 1 :ec 1 :el 1}
                     {:bl 2 :bc 2 :ec 2 :el 2})
  # =>
  true

  (p/sym-at-or-before? {:bl 2 :bc 2 :ec 2 :el 2}
                     {:bl 1 :bc 1 :ec 1 :el 1})
  # =>
  false

  (p/sym-at-or-before? {:bl 1 :bc 1 :ec 8 :el 1}
                     {:bl 1 :bc 1 :ec 8 :el 1})
  # =>
  true
  ...
  (def [ok? result]
    (protect (p/sym-at-or-before? {:bl 1 :bc 1 :el 1 :ec 3}
                                {:bl 1 :bc 2 :el 1 :ec 7})))
...
(defn p/rename
  ...
  (def def-descs (reduce p/analyze-defish @[] sym-zlocs))
    ...
    (when-let [found
               (match (j/node cur-zloc) [:symbol curr-loc name]
                 (when-let [def-name-loc (get sym-tbl name)
                            _ (p/sym-at-or-before? def-name-loc curr-loc)]
                   name))]
